...
from vocoder.waveglow import inference as wg_vocoder
from vocoder.melgan.scripts import generate_from_folder as mel_vocoder
#from vocoder.melgan import inference as mel_vocoder
...
class Toolbox:
    ...
    def vocode(self):
        ...
        if self.ui.current_vocoder_fpath is not None:
            self.ui.log("")
            wav = vocoder.infer_waveform(spec, progress_callback=vocoder_progress) ##此处将合成的mel谱传入vocoder
        else:
    ...
    def init_synthesizer(self):
        ...
        checkpoints_dir = model_fpath.joinpath("taco_pretrained")
        self.ui.log("Loading the synthesizer %s... " % model_fpath)
        self.ui.set_loading(1)
        start = timer()
        self.synthesizer = Synthesizer(checkpoints_dir, low_mem=False)
        self.ui.log("Done (%dms)." % int(1000 * (timer() - start)), "append")
        self.ui.set_loading(0)
    ...
    def init_vocoder(self):
        ...
        elif model_fpath.name[0] == "w":
            vocoder = wg_vocoder
            self.ui.log("set waveglow as vocoder")
        elif model_fpath.name[0] == "b":
            vocoder = mel_vocoder
            self.ui.log("set melgan as vocoder")
        else:
            vocoder = rnn_vocoder
