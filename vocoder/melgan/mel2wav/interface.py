from vocoder.melgan.mel2wav.modules import Generator, Audio2Mel
...
def load_model(mel2wav_path, device=get_default_device()):
    ...
    with open('F:/cuc/2021/Realtime-tacotron-Chinese/vocoder/saved_models/pretrained/args.yml', "r") as f:
        args = yaml.load(f, Loader=yaml.FullLoader)
    netG = Generator(args['n_mel_channels'], args['ngf'], args['n_residual_layers']).to(device)
    netG.load_state_dict(torch.load('F:/cuc/2021/Realtime-tacotron-Chinese/vocoder/saved_models/pretrained/best_netG.pt', map_location=device))
    return netG
